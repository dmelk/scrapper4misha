...
export class DinamotoScrapper implements ScrapperInterface {
  ...
  public async startScrapping(
    ...
    for (let i = 0; i < catalogList.length; i++) {
      ...
      for (
        let currPage = 1;
        currPage <= maxPage;
        currPage += DinamotoScrapper.PAGE_STEP
      ) {
  ...
  private async loadProduct(
    ...
    $(
      'section.d-md-none > div > div > div > div > div > div.thumbImg > img',
    ).each((i, img) => {
