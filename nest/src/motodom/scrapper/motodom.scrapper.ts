...
export class MotodomScrapper implements ScrapperInterface {
  ...
  private static readonly COMBINATION_URL =
    'index.php?route=journal3/price&popup=0';
  ...
  public async startScrapping(
    ...
    for (let i = 0; i < catalogList.length; i++) {
      ...
      for (
        let currPage = 1;
        currPage <= maxPage;
        currPage += MotodomScrapper.PAGE_STEP
      ) {
  ...
  private async loadCatalogPage(
    ...
    try {
      ...
      const products = $(
        'div.main-products > div > div > div.caption > div.name > a',
      );
  ...
  private async loadProduct(
    ...
    const descriptionBlocks = $(
      'div.product_tabs-default > div.tab-content > div',
    );
    ...
    const images = $(
      'div.main-image > div.swiper-container > div.swiper-wrapper > div.swiper-slide > img',
    );
    images.each((i, img) => {
      productInfo.photos.push(img.attribs['data-largeimg']);
    });
    ...
    if (options.length === 0) {
      ...
      options.each((i, option) => {
        ...
        option$('div.radio > label > input').each((idx, input) => {
          if (idx === 0) {
          ...
          variantData.values.push(input.attribs.value);
        });

        option$('div.radio > label > span').each((idx, span) => {
      ...
      for (let i = 0; i < combinations.length; i++) {
        const combination = combinations[i],
          combinationUrl = MotodomScrapper.BASE_URL.concat(
            MotodomScrapper.COMBINATION_URL,
          ),
          combinationData: any = {
            quantity: 1,
        ...
        combination.forEach((variantData) => {
          combinationData.option[variantData.optionId] = variantData.value;
        });

        const combinationResult = await got.post(combinationUrl, {
        ...
        if (jsonResponse.response) {
          if (jsonResponse.response.in_stock || combination.length === 1) {
            if (jsonResponse.response.price) {
              productInfo.prices.push(parseFloat(jsonResponse.response.price));
